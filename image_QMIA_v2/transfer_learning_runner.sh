...
QMIA_ARCHITECTURE=cifar-resnet-18 #cifar-resnet-18 #convnext-tiny #

# Set these variables
BASE_DATASET=cinic10/0_16 #imagenet-1k/0_16 #
ATTACK_DATASET=cinic10/0_16 #cifar100/0_16 #imagenet-1k/0_16 #
DROPPED_CLASSES=''

# Train base model
python train_base.py --dataset=$BASE_DATASET --architecture=$BASE_ARCHITECTURE --model_root=$MODEL_DIR --data_root=$DATA_DIR \
--batch_size=32 --scheduler=step --scheduler_step_gamma=0.2 --scheduler_step_fraction=0.3 --lr=0.1 --weight_decay=5e-4 --image_size=32 --epochs=200 --rerun
